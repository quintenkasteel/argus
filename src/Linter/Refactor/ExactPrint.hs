...
-- |
-- Module      : Linter.Refactor.ExactPrint
...
runTransformIO :: MonadIO m => TransformT m a -> TransformState -> m (a, TransformState)
...
replaceExpr span newText = modify $ \s ->
  s { tsSource = replaceInSource (tsSource s) span newText
    , tsModified = True
    }
...
removeDecl span = modify $ \s ->
  s { tsSource = removeFromSource (tsSource s) span
    , tsModified = True
    }
...
applyEdit source (FixEdit span newText) =
  replaceInSource source span newText
...
replaceInSource source SrcSpan{..} newText =
  let sourceLines = T.lines source
      beforeLines = take (srcSpanStartLine - 1) sourceLines
      afterLines = drop srcSpanEndLine sourceLines

      -- Handle the lines that are partially affected
      startLine = if srcSpanStartLine > 0 && srcSpanStartLine <= length sourceLines
                  then sourceLines !! (srcSpanStartLine - 1)
                  else ""
      endLine = if srcSpanEndLine > 0 && srcSpanEndLine <= length sourceLines
                then sourceLines !! (srcSpanEndLine - 1)
                else ""

      beforeCol = T.take (srcSpanStartCol - 1) startLine
      afterCol = T.drop (srcSpanEndCol - 1) endLine
...
removeFromSource source span = replaceInSource source span ""
