...
composeIdRight f xs = map (f . id) xs

-- id . f ==> f
composeIdLeft :: (Int -> Int) -> [Int] -> [Int]
composeIdLeft f xs = map (id . f) xs

-- More complex: id . id ==> id
doubleIdCompose :: [Int] -> [Int]
doubleIdCompose = map (id . id)
...
idApplySimple x = id x

idApplyList :: [Int] -> [Int]
idApplyList xs = map id xs

idApplyComposed :: [Int] -> [Int]
idApplyComposed xs = filter (id even) xs
...
complexChain1 xs = map (id . not . id . even . id) xs

complexChain2 :: [String] -> [Int]
complexChain2 xs = map (\s -> (length . id . words . id) s) xs
...
mixedPattern1 xs = map (id . (\x -> x) . (+1)) xs

mixedPattern2 :: [String] -> [Int]
mixedPattern2 xs = map (\s -> (id . length) s) xs

-- const and id together
mixedConstId :: [a] -> [Int]
mixedConstId xs = map (id . const 5 . id) xs
