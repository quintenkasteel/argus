...
lengthZeroCompare xs = not (null xs)
...
combineResults xs = fold xs

-- Manual traversal
applyActions :: Monad m => (a -> m b) -> [a] -> m [b]
applyActions action items = traverse action items
...
processAndDiscard processor items = void (traverse processor items)

-- Traverse with flipped arguments
forEachItem :: Monad m => [a] -> (a -> m b) -> m [b]
forEachItem items action = for items action
