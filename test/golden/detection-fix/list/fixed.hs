...
flatMapPattern f xs = concatMap f xs
...
isEmptyList :: [a] -> Bool
isEmptyList items = null items

-- Check if list has elements the slow way
hasElements :: [a] -> Bool
hasElements items = not (null items)

-- Flatten nested lists inefficiently
flattenAll :: [[a]] -> [a]
flattenAll lists = concatMap id lists
