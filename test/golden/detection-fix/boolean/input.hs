...
boolCheck1 x = if x then True else False

-- if x then False else True -> not x
boolCheck2 :: Bool -> Bool
boolCheck2 x = if x then False else True

-- not (not x) -> x
doubleNot :: Bool -> Bool
doubleNot x = not (not x)

-- x == True -> x
eqTrueCheck :: Bool -> Bool
eqTrueCheck x = x == True

-- x == False -> not x
eqFalseCheck :: Bool -> Bool
eqFalseCheck x = x == False
...
checkPermissions isAdmin isOwner isGuest =
  if isAdmin == True then True else isOwner
...
confusingLogic a b = not (not a) && b
...
complexBoolean a b c = (a == True) && (not (not b)) || (c == False)
