...
calculateSum nums = getSum (mconcatMap Sum nums)

-- Product with wrapper
calculateProduct :: [Int] -> Int
calculateProduct nums = getProduct (mconcatMap Product nums)

-- Check all conditions
allTrue :: [Bool] -> Bool
allTrue conditions = getAll (mconcatMap All conditions)

-- Check any condition
anyTrue :: [Bool] -> Bool
anyTrue conditions = getAny (mconcatMap Any conditions)

-- Combine with identity
appendToEmpty :: String -> String
appendToEmpty s = mempty <> s
