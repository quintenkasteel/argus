...
conditionalAction flag = if flag then putStrLn "Yes" else return ()
...
compareTrue x = x == True

-- Comparison with False
compareFalse :: Bool -> Bool
compareFalse x = x == False

-- Redundant if True/False
redundantIf1 :: Bool -> Bool
redundantIf1 x = if x then True else False

-- Another redundant if pattern
redundantIf2 :: Bool -> Bool
redundantIf2 x = if x then False else True

-- Redundant map id
mapIdentity :: [a] -> [a]
mapIdentity xs = map id xs

-- Redundant fmap id
fmapIdentity :: Functor f => f a -> f a
fmapIdentity x = fmap id x
...
returnUnit :: Bool -> IO ()
returnUnit c = if c then return () else putStrLn "No"
