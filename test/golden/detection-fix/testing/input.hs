...
manyExamples :: Spec
manyExamples = do
  it "reverse twice is identity 1" $ reverse (reverse [1]) `shouldBe` [1]
  it "reverse twice is identity 2" $ reverse (reverse [1,2]) `shouldBe` [1,2]
  it "reverse twice is identity 3" $ reverse (reverse [1,2,3]) `shouldBe` [1,2,3]
